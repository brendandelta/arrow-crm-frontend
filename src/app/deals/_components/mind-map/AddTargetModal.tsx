...
interface AddTargetModalProps {
  dealId: number;
  onClose: () => void;
  onSave: (data: { deal_id: number; target_type: string; target_id: number; role: string; status: string }) => Promise<void>;
}
...
export function AddTargetModal({ dealId, onClose, onSave }: AddTargetModalProps) {
  const [organizations, setOrganizations] = useState<Organization[]>([]);
  const [loadingOrgs, setLoadingOrgs] = useState(true);
  const [targetType, setTargetType] = useState<"Organization" | "Person">("Organization");
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white border border-slate-200 rounded-xl w-full max-w-md mx-4 max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between p-4 border-b border-slate-100">
          <h2 className="text-base font-semibold text-slate-900">Add Target</h2>
          <button
            onClick={onClose}
